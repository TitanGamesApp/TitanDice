...
export default function StakeCard() {
  ...
  const { titanGMBalance, refetchBurn, refetchStake } = useDice();
  ...
  const handleStake = async () => {
    ...
    try {
      let allowance = await publicClient?.readContract({
        ...
        args: [account.address, TitanGamesStakingContract],
      });

      if (BigInt(Number(allowance)) < parseEther(titanGMAmount.toString())) {
        hash = await writeContractAsync({
          ...
          args: [
            TitanGamesStakingContract,
            parseEther(titanGMAmount.toString()),
          ],
        });
